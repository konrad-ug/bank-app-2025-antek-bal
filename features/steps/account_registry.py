...
def create_account(context, first_name, last_name, pesel):
    json_body = {"first_name": f"{first_name}",
                 "last_name": f"{last_name}",
                 "pesel": pesel
                 }
    create_resp = requests.post(URL + "/api/personal_accounts", json=json_body)
    assert create_resp.status_code == 201


@step('Account registry is empty')
def clear_account_registry(context):
    response = requests.get(URL + "/api/personal_accounts")
    accounts = response.json()
    for account in accounts:
        pesel = account["pesel"]
        requests.delete(URL + f"/api/personal_accounts/{pesel}")
...
def delete_account(context, pesel):
    response = requests.delete(URL + f"/api/personal_accounts/{pesel}")
    assert response.status_code == 200
...
def update_field(context, field, pesel, value):
    ...
    response = requests.patch(URL + f"/api/personal_accounts/{pesel}", json=json_body)
    assert response.status_code == 200
